...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function spawnEnemy() {
        ...
        if (gameActive) {
            ...
            spawnTimeoutId = setTimeout(spawnEnemy, nextSpawnDelay);
        }
    }
        ...
        if (enemy.behaviorTimer <= 0) {
            ...
            switch (enemy.type) {
                ...
                case 'scooter':
                    ...
                    // Keep scooter in bounds
                    enemy.x = Math.max(CONFIG.ROAD_MARGIN, Math.min(enemy.x, canvas.width - enemy.width - CONFIG.ROAD_MARGIN));
                    break;
    ...
    // Expose resetGame to global scope for the button onclick
    window.resetGame = resetGame;

    // Start the game
    initGame();
    
    console.log('Cairo Chaos: Traffic Survival initialized!');
});
